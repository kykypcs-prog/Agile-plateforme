        ...
        <div className="p-8 animate-fadeInUp">
            ...
            <div className="space-y-6">
                ...
                {[
                  ...
                  <div key={i} className="bg-white rounded-2xl p-5 shadow-lg card-hover border border-gray-100 relative overflow-hidden group">
                    ...
                    <div className="relative z-10">
                      ...
                      <div className="mt-2 flex items-center gap-1 text-xs text-green-600"><TrendingUp size={12} /><span>+5% ce mois</span></div>
                    </div>
                  </div>
                ))}
              </div>
              <div className="grid grid-cols-2 gap-6">
                <div className="bg-white rounded-2xl p-6 shadow-lg card-hover border border-gray-100">
                  <h3 className="text-sm font-bold text-gray-700 mb-4 flex items-center gap-2"><PieChart size={16} className="text-emerald-500" /> État de mes tâches</h3>
                ...
                <div className="bg-white rounded-2xl p-6 shadow-lg card-hover border border-gray-100">
                  <h3 className="text-sm font-bold text-gray-700 mb-4 flex items-center gap-2"><Clock size={16} className="text-indigo-500" /> Mes Projets récents</h3>
                  {projects.length === 0 ? (
                    <div className="text-center text-gray-300 py-8"><FolderKanban size={48} className="mx-auto mb-3 opacity-30" /><p className="text-xs">Vous n'êtes assigné à aucun projet</p></div>
                  ) : (
                    <div className="space-y-2">
                      {projects.slice(0, 4).map((project) => (
                        <div key={project.id} className="flex justify-between items-center py-2.5 px-3 rounded-xl hover:bg-gray-50 transition-all group">
                          <div className="flex items-center gap-2"><div className="w-2 h-2 bg-gradient-to-r from-emerald-400 to-teal-400 rounded-full animate-pulse"></div><p className="text-sm font-semibold text-gray-700 group-hover:text-emerald-600">{project.name}</p></div>
                          <button onClick={() => navigate(`/project/${project.id}`)} className="text-emerald-600 text-xs hover:underline flex items-center gap-1">Voir <ChevronRight size={12} /></button>
                        </div>
                      ))}
                    </div>
                  )}
          ...
          {activePage === 'projects' && (
            <div className="space-y-6">
              ...
              {projects.length === 0 ? (
                <div className="bg-white rounded-2xl p-16 text-center text-gray-400 shadow-lg"><FolderKanban size={48} className="mx-auto mb-3 opacity-30" /><p className="text-sm">Vous n'êtes assigné à aucun projet</p></div>
              ) : (
                <div className="grid grid-cols-3 gap-5">
                  {projects.map((project) => (
                    <div key={project.id} className="bg-white rounded-2xl p-5 shadow-lg card-hover border border-gray-100 group">
                      <div className="flex justify-between items-start mb-3"><h3 className="text-sm font-bold text-gray-800 group-hover:text-emerald-600">{project.name}</h3><span className="bg-gradient-to-r from-emerald-400 to-teal-400 text-white text-xs px-2 py-0.5 rounded-full shadow-sm">Actif</span></div>
                      <p className="text-xs text-gray-400 mb-4 line-clamp-2">{project.description || 'Aucune description'}</p>
                      <div className="flex justify-between items-center pt-3 border-t border-gray-100"><span className="text-xs text-gray-400">{project.sprints?.length || 0} sprint(s)</span><button onClick={() => navigate(`/project/${project.id}`)} className="text-emerald-600 text-xs hover:underline flex items-center gap-1">Voir <ChevronRight size={12} /></button></div>
                    </div>
                  ))}
                </div>
              )}
          ...
          {activePage === 'sprints' && (
            <div className="space-y-6">
              ...
              <div className="bg-white rounded-2xl p-5 shadow-lg border border-gray-100">
                ...
                <select className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm" onChange={handleProjectChange} value={selectedProject}>
                  <option value="">-- Choisir un projet --</option>
                  {projects.map((p) => (<option key={p.id} value={p.id}>{p.name}</option>))}
                </select>
              </div>
              {!selectedProject ? (
                <div className="bg-white rounded-2xl p-16 text-center text-gray-400 shadow-lg"><Timer size={48} className="mx-auto mb-3 opacity-30" /><p className="text-sm">Sélectionnez un projet pour voir ses sprints</p></div>
              ) : sprints.length === 0 ? (
                <div className="bg-white rounded-2xl p-16 text-center text-gray-400 shadow-lg"><Calendar size={48} className="mx-auto mb-3 opacity-30" /><p className="text-sm">Aucun sprint pour ce projet</p></div>
              ) : (
                <div className="grid grid-cols-3 gap-5">
                  {sprints.map((sprint) => (
                    <div key={sprint.id} className="bg-white rounded-2xl p-5 shadow-lg card-hover border border-gray-100 group">
                      <div className="flex justify-between items-start mb-3"><h3 className="text-sm font-bold text-gray-800 group-hover:text-violet-600">{sprint.name}</h3><span className="bg-gradient-to-r from-violet-400 to-purple-400 text-white text-xs px-2 py-0.5 rounded-full shadow-sm">Actif</span></div>
                      <p className="text-xs text-gray-400 flex items-center gap-1">📅 {new Date(sprint.startDate).toLocaleDateString()} → {new Date(sprint.endDate).toLocaleDateString()}</p>
                      <div className="mt-4 pt-3 border-t border-gray-100"><span className="text-xs text-gray-400">{sprint.tasks?.length || 0} tâche(s)</span></div>
                    </div>
                  ))}
                </div>
              )}
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-5">
                ...
                <div className="bg-white rounded-2xl p-5 shadow-lg border border-gray-100">
                  ...
                  <select className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm" onChange={(e) => { setSelectedSprint(e.target.value); fetchMyTasks(e.target.value) }} value={selectedSprint} disabled={!selectedProject}>
                ...
                <div className="grid grid-cols-3 gap-5">
                  {[
                    ...
                    <div key={col.status} className={`${col.bg} rounded-2xl p-4 shadow-lg border ${col.border} card-hover`}>
                      <div className="flex items-center gap-2 mb-4"><div className={`w-2 h-2 rounded-full ${col.dot} animate-pulse`}></div><h3 className={`text-sm font-bold ${col.color}`}>{col.label}</h3><span className="ml-auto bg-white text-gray-500 text-xs px-2 py-0.5 rounded-full border">{tasks.filter(t => t.status === col.status).length}</span></div>
                      <div className="space-y-3 max-h-[500px] overflow-y-auto pr-1">
                        {tasks.filter(t => t.status === col.status).map((task) => (
                          <div key={task.id} className="bg-white rounded-xl p-3 shadow-md border border-gray-100 hover:shadow-lg transition-all">
                            ...
                            {task.assignee && (<div className="flex items-center gap-1 mb-2"><div className="w-4 h-4 bg-gradient-to-r from-emerald-500 to-teal-500 rounded-full flex items-center justify-center"><span className="text-white text-[8px]">{task.assignee.name.charAt(0)}</span></div><span className="text-xs text-gray-500">{task.assignee.name}</span></div>)}
                            <div className="flex gap-1 mt-2 pt-2 border-t border-gray-50">
                              {col.status !== 'TODO' && (<button onClick={() => handleMoveTask(task.id, col.status === 'IN_PROGRESS' ? 'TODO' : 'IN_PROGRESS')} className="text-xs bg-gray-100 hover:bg-gray-200 px-2 py-1 rounded-lg">← Reculer</button>)}
                              {col.status !== 'DONE' && (<button onClick={() => handleMoveTask(task.id, col.status === 'TODO' ? 'IN_PROGRESS' : 'DONE')} className="text-xs bg-emerald-100 hover:bg-emerald-200 px-2 py-1 rounded-lg text-emerald-600">Avancer →</button>)}
                            </div>
                          </div>
                        ))}
                        {tasks.filter(t => t.status === col.status).length === 0 && (<div className="text-center text-gray-300 py-8 text-xs">Aucune tâche</div>)}
                      </div>
                    </div>
                  ))}
                </div>
              ) : (
                <div className="bg-white rounded-2xl p-16 text-center text-gray-400 shadow-lg"><Kanban size={48} className="mx-auto mb-3 opacity-30" /><p className="text-sm">Sélectionnez un projet et un sprint</p></div>
              )}
            </div>
          )}
          ...
          {activePage === 'mytasks' && (
            <div className="space-y-6">
              ...
              {selectedSprint ? (
                tasks.length === 0 ? (
                  <div className="bg-white rounded-2xl p-16 text-center text-gray-400 shadow-lg"><ClipboardList size={48} className="mx-auto mb-3 opacity-30" /><p className="text-sm">Aucune tâche assignée dans ce sprint</p></div>
                ) : (
                  <div className="grid grid-cols-3 gap-5">
                    {[
                      { status: 'TODO', label: 'À Faire', color: 'text-amber-600', bg: 'bg-amber-50', border: 'border-amber-200', dot: 'bg-amber-500' },
                ...
                <div className="bg-white rounded-2xl p-16 text-center text-gray-400 shadow-lg"><ClipboardList size={48} className="mx-auto mb-3 opacity-30" /><p className="text-sm">Sélectionnez un projet et un sprint</p></div>
              )}
            </div>
          )}
          ...
          {activePage === 'members' && (
            <div className="space-y-6">
              ...
              <div className="bg-white rounded-2xl p-5 shadow-lg border border-gray-100">
                <label className="text-xs text-gray-500 font-semibold mb-2 block uppercase">SÉLECTIONNER UN PROJET</label>
                <select className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm" onChange={(e) => { setSelectedProject(e.target.value); fetchMembers(e.target.value) }} value={selectedProject}>
                  <option value="">-- Choisir un projet --</option>
                  {projects.map((p) => (<option key={p.id} value={p.id}>{p.name}</option>))}
                </select>
              </div>
              {!selectedProject ? (
                <div className="bg-white rounded-2xl p-16 text-center text-gray-400 shadow-lg"><Users size={48} className="mx-auto mb-3 opacity-30" /><p className="text-sm">Sélectionnez un projet pour voir ses membres</p></div>
              ) : members.length === 0 ? (
                <div className="bg-white rounded-2xl p-16 text-center text-gray-400 shadow-lg"><UserPlus size={48} className="mx-auto mb-3 opacity-30" /><p className="text-sm">Aucun membre pour ce projet</p></div>
              ) : (
                <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
                  <table className="w-full">
                    <thead><tr className="border-b border-gray-100 bg-gray-50/50"><th className="text-left px-6 py-3 text-xs text-gray-500 font-semibold uppercase">NOM</th><th className="text-left px-6 py-3 text-xs text-gray-500 font-semibold uppercase">EMAIL</th><th className="text-left px-6 py-3 text-xs text-gray-500 font-semibold uppercase">RÔLE</th><th className="text-left px-6 py-3 text-xs text-gray-500 font-semibold uppercase">STATUT</th></tr></thead>
                    <tbody>
                      {members.map((member) => (
                        <tr key={member.id} className="border-b border-gray-50 hover:bg-rose-50/30 transition-all">
                          <td className="px-6 py-4"><div className="flex items-center gap-2"><div className={`w-7 h-7 rounded-full flex items-center justify-center text-white text-xs font-bold shadow-sm ${member.user.id === user?.id ? 'bg-gradient-to-r from-emerald-500 to-teal-600' : 'bg-gradient-to-r from-gray-400 to-gray-500'}`}>{member.user.name.charAt(0)}</div><span className="text-sm font-semibold text-gray-800">{member.user.name}{member.user.id === user?.id && <span className="ml-1 text-xs text-emerald-600">(Moi)</span>}</span></div></td>
                          <td className="px-6 py-4 text-sm text-gray-500">{member.user.email}</td>
                          <td className="px-6 py-4"><span className={`text-xs px-2 py-1 rounded-full font-semibold ${member.user.role === 'ADMIN' ? 'bg-gradient-to-r from-rose-100 to-pink-100 text-rose-700 border border-rose-200' : member.user.role === 'CHEF' ? 'bg-gradient-to-r from-indigo-100 to-purple-100 text-indigo-700 border border-indigo-200' : 'bg-gradient-to-r from-emerald-100 to-teal-100 text-emerald-700 border border-emerald-200'}`}>{member.user.role}</span></td>
                          <td className="px-6 py-4"><span className="bg-gradient-to-r from-emerald-100 to-teal-100 text-emerald-700 text-xs px-3 py-1 rounded-full border border-emerald-200 shadow-sm">Actif</span></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
